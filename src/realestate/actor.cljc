(ns realestate.actor
  "RealEstateActor — the ISCO-08 3334 community realestate actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one realestate operation request
  ...
  The unconditional invariant: the RealEstateAdvisor can never
  directly commit a record the RealEstateGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [realestate.advisor :as advisor]
            [realestate.governor :as governor]
            [realestate.store :as store]))

(defn build-graph
  "Build a compiled RealEstateActor graph. `store` implements
  `realestate.store/Store`. `advisor` implements
  `realestate.advisor/Advisor` (defaults to `mock-advisor`).
